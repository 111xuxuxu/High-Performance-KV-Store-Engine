...
int main() {
#if ENABLE_KEY_CHAR
	char *keyArray[10] = {"king","queen","jack","tom","jerry","alice","bob","candy","david","lucy"};
	char *valueArray[10] = {"1king","2queen","3jack","4tom","5jerry","6alice","7bob","8candy","9david","10lucy"};
...
// 辅助函数：递归释放节点
void _kvs_rbtree_free_node(kvs_rbtree_t *inst, rbtree_node *node) {
    // 如果走到叶子节点（NIL），直接返回
    if (node == inst->nil) {
        return;
    }
    ...
    // 1. 先递归释放左子树
    _kvs_rbtree_free_node(inst, node->left);
    // 2. 再递归释放右子树
    _kvs_rbtree_free_node(inst, node->right);
...
int kvs_rbtree_destory(kvs_rbtree_t *inst) {
    ...
    if (inst->root != inst->nil) {
        _kvs_rbtree_free_node(inst, inst->root);
    }
